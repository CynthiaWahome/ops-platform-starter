...
func TestWorkItemHandlerCreateReturnsCreatedItem(t *testing.T) {
	...
	service := workitems.NewService(workitems.NewMemoryStore(), workitems.NewMemoryStatusHistoryStore(), workitems.NewMemoryAssignmentStore(), workitems.NewMemoryAssignmentHistoryStore(), notifications.NewService(notifications.NewMemoryStore()), nil, nil).WithClock(func() time.Time {
